...
def _strategy_has_pending_live_trade(strategy_state: LiveStrategyState | None) -> bool:
    return bool(strategy_state is not None and strategy_state.pending_live_slug)


def _managed_live_strategy_ids(
...
def _live_trade_log_upsert_key(row: dict[str, Any]) -> tuple[str, str] | None:
...
def _merge_live_trade_log_rows(existing: dict[str, str], incoming: dict[str, Any], fieldnames: list[str]) -> dict[str, Any]:
...
def append_trade_log(path: Path, record: TradeRecord) -> None:
    ...
    upsert_key = _live_trade_log_upsert_key(row) if _row_has_result(row) else None
    if upsert_key is not None and path.exists() and not write_header:
        ...
        for index in range(len(existing_rows) - 1, -1, -1):
            ...
            if _live_trade_log_upsert_key(existing_row) != upsert_key:
                continue
            if _row_has_result(existing_row):
                continue
            existing_rows[index] = _merge_live_trade_log_rows(existing_row, row, fieldnames)
