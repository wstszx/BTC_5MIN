...
def test_place_live_order_strategy7_revalidates_low_order_book_price_before_skipping(tmp_path, monkeypatch):
    class _RefreshingStrategy7MarketClient(_LiveMarketClient):
        def __init__(self):
            self.quote_calls = 0
    ...
    decisions: list[SideDecision] = [
        SideDecision(side="UP", signal_open_up_price=0.435, signal_current_up_price=0.505, signal_delta=0.07),
        SideDecision(side="UP", signal_open_up_price=0.435, signal_current_up_price=0.51, signal_delta=0.075),
    ]
    ...
    market_client = _RefreshingStrategy7MarketClient()
...
def test_place_live_order_audits_official_fill_below_min_entry_price(tmp_path, monkeypatch):
    class _DecisionPriceLiveMarketClient(_LiveMarketClient):
        def quote_from_market(self, _market):
            return MarketQuote(
                slug="btc-updown-5m-test",
    ...
    stub_clob = _StubClobClient(
        ...
        trade_payloads={
            "oid-123": [
                _confirmed_trade("oid-123", size=2.083333, price=0.48),
            ],
        },
    )
    state_path = tmp_path / "state.json"
    ...
    assert result["status"] == "submitted"
    assert result["raw_price"] == pytest.approx(0.48)
    assert rows[-1]["fill_source"] == "official_fill_price_below_min_entry"
    assert any("official_fill_price_below_min_entry" in message for message in messages)
    assert any("min_entry_price=0.5" in message for message in messages)
...
def test_run_live_trading_recreates_cached_clob_client_after_retryable_submit_error(tmp_path, monkeypatch):
    stop_event = threading.Event()
    attempts = {"sleep": 0}
    created_clients: list[_TransientSubmitClobClient] = []
    ...
    def fake_create_client(_cfg):
        client = _TransientSubmitClobClient(balance_payload={"available": 3.0})
...
def test_run_live_trading_settlement_preserves_low_fill_source_marker(tmp_path, monkeypatch):
    stop_event = threading.Event()
    stub_clob = _StubClobClient(
        order_payloads={"oid-prev": {"status": "filled"}},
        trade_payloads={"oid-prev": [_confirmed_trade("oid-prev", size=2.0833333333333335, price=0.48)]},
        balance_payload={"available": 10.0},
    )
    state_path = tmp_path / "live_state.json"
    log_path = tmp_path / "live_orders.csv"
        ...
        json.dumps(
            {
                "live_strategies": {
                    "10": {
                        ...
                        "pending_live_slug": "btc-updown-5m-prev",
                        "pending_live_side": "UP",
                        "pending_live_price": 0.496128,
                        "pending_live_order_size": 2.0833333333333335,
                        "pending_live_order_cost": 1.0336,
                        "pending_live_expected_profit": 1.05,
                        "pending_live_end_time": "2026-04-02T00:00:00+00:00",
                        "pending_live_order_id": "oid-prev",
                        "pending_live_raw_price": 0.48,
                        "pending_live_raw_order_cost": 1.0,
                        "pending_live_fee": 0.0336,
                    }
                }
            }
        ),
    ...
    append_trade_log(
        ...
        TradeRecord(
            ...
            side="UP",
            price=0.496128,
            order_size=2.0833333333333335,
            order_cost=1.0336,
            expected_profit=1.05,
            order_id="oid-prev",
            fill_source="official_fill_price_below_min_entry",
            raw_price=0.48,
            raw_order_cost=1.0,
            fee=0.0336,
        ),
    ...
    assert result["status"] == "stopped"
    assert settled["raw_price"] == "0.48"
    assert settled["result"] == "DOWN"
    assert settled["fill_source"] == "official_fill_price_below_min_entry"
