...
def test_auto_reconcile_live_ledger_counts_provisional_loss_for_risk_state(tmp_path: Path):
    class StubClient:
        def get_event_by_slug(self, slug: str):
            return {
                "eventMetadata": {"priceToBeat": "78627.34"},
                "markets": [{"slug": slug, "closed": True}],
                "closed": True,
            }
    ...
    rows = list(csv.DictReader(live_csv.open(newline="", encoding="utf-8")))
    payload = json.loads(state_path.read_text(encoding="utf-8"))
    assert changed == 0
...
def test_auto_reconcile_live_ledger_resolves_provisional_loss_when_final_price_arrives(tmp_path: Path):
    class StubClient:
        def get_event_by_slug(self, slug: str):
            return {
                "eventMetadata": {
...
def test_dashboard_live_recent_orders_does_not_reconcile_from_terminal_price_only(tmp_path: Path, monkeypatch):
    ...
    try:
        ...
        assert row["result"] == "PROVISIONAL_LOSS"
        assert row["resolved_expected_result"] == ""
        assert row["result_check_status"] == "official_pending"
        assert persisted_row["result"] == "PROVISIONAL_LOSS"
        assert persisted_row["trade_pnl"] == "-1.19999936"
    finally:
        state.close()
        os.chdir(old_cwd)


def test_auto_reconcile_live_ledger_downgrades_existing_result_without_final_price(tmp_path: Path):
    class StubClient:
        def get_event_by_slug(self, slug: str):
            return {
                "id": "evt-live",
    ...
    with live_csv.open("w", newline="", encoding="utf-8") as handle:
        ...
        writer.writerow(
            {
                "timestamp": "2026-05-05T03:15:04+00:00",
                "mode": "live",
                "round_index": "680",
                "strategy": "7",
                "entry_timing": "OPEN",
                "event_slug": "btc-updown-5m-1777950900",
                "start_time": "2026-05-05T03:15:00+00:00",
                "end_time": "2026-05-05T03:20:00+00:00",
                "side": "DOWN",
                "price": "0.44",
                "order_size": "5.021275",
                "order_cost": "2.209361",
                "expected_profit": "2.811914",
                "result": "DOWN",
                "trade_pnl": "2.811914",
                "cash_pnl": "2.811914",
                "recovery_loss": "0.0",
...
def test_auto_reconcile_live_ledger_uses_market_endpoint_final_price_pair(tmp_path: Path):
    class StubClient:
        def get_event_by_slug(self, slug: str):
            return {
                "id": "evt-live",
...
def test_live_recent_orders_does_not_reconcile_without_final_price(tmp_path: Path, monkeypatch):
    ...
    try:
        ...
        assert row["result_check_status"] == "official_pending"
        assert persisted_row["result"] == "PROVISIONAL_LOSS"
        assert persisted_row["trade_pnl"] == "-1.19999932"
        assert state_payload["live_strategies"]["7"]["cash_pnl"] == -1.19999932
    finally:
        state.close()
        os.chdir(old_cwd)
...
def test_live_recent_orders_auto_reconciles_mismatched_result(tmp_path: Path, monkeypatch):
    old_cwd = Path.cwd()
    os.chdir(tmp_path)
    ...
    try:
        ...
        (logs_dir / "live_orders.csv").write_text(
            "timestamp,mode,round_index,strategy,entry_timing,event_slug,start_time,end_time,side,price,order_size,order_cost,expected_profit,result,trade_pnl,cash_pnl,recovery_loss,consecutive_losses,stop_loss_triggered,skip_reason,signal_open_up_price,signal_current_up_price,signal_threshold,signal_delta,signal_locked,signal_reason,experiment_id\n"
            "2026-05-01T09:35:01+00:00,live,10,4,OPEN,btc-updown-5m-1777628100,2026-05-01T09:35:00+00:00,2026-05-01T09:40:00+00:00,UP,0.55,4.0,2.2,1.8,UP,1.8,1.8,0.0,0,False,,,,,,False,,\n",
            encoding="utf-8",
        )
        (logs_dir / "live_session_state.json").write_text(
            json.dumps(
        ...
        payload = state.get_live_recent_orders_payload(limit=10, strategy=4)
        row = payload["rows"][0]
        ...
        backups = list(logs_dir.glob("live_orders_backup_before_reconcile_*.csv"))
...
def test_live_recent_orders_auto_reconciles_provisional_loss(tmp_path: Path, monkeypatch):
    ...
    try:
        ...
        (logs_dir / "live_orders.csv").write_text(
            "timestamp,mode,round_index,strategy,entry_timing,event_slug,start_time,end_time,side,price,order_size,order_cost,expected_profit,result,trade_pnl,cash_pnl,recovery_loss,consecutive_losses,stop_loss_triggered,skip_reason,signal_open_up_price,signal_current_up_price,signal_threshold,signal_delta,signal_locked,signal_reason,experiment_id\n"
            "2026-05-01T09:35:01+00:00,live,10,4,OPEN,btc-updown-5m-1777628100,2026-05-01T09:35:00+00:00,2026-05-01T09:40:00+00:00,UP,0.55,4.0,2.2,1.8,PROVISIONAL_LOSS,-2.2,-2.2,2.2,1,False,,,,,,False,,\n",
            encoding="utf-8",
        )
        (logs_dir / "live_session_state.json").write_text(
            json.dumps(
        ...
        assert row["result_check_status"] == "match"
        assert rows[0]["result"] == "UP"
        assert rows[0]["trade_pnl"] == "1.8"
        assert rows[0]["cash_pnl"] == "1.8"
        assert rows[0]["recovery_loss"] == "0.0"
        assert rows[0]["consecutive_losses"] == "0"
        assert state_payload["live_strategies"]["4"]["cash_pnl"] == 1.8
        assert state_payload["live_strategies"]["4"]["recovery_loss"] == 0.0
        assert state_payload["live_strategies"]["4"]["consecutive_losses"] == 0
        assert backups
    finally:
        state.close()
        os.chdir(old_cwd)
...
def test_dashboard_config_payload_includes_paper_strategy_ids(tmp_path: Path):
    ...
    try:
        ...
        assert payload['select_options']['STRATEGY_ID'] == SUPPORTED_STRATEGY_OPTIONS
        assert payload['select_options']['STRATEGY_IDS'] == SUPPORTED_STRATEGY_OPTIONS
...
def test_dashboard_config_payload_includes_strategy9_fields(tmp_path: Path):
    state = DashboardState(env_file=tmp_path / '.env.dashboard')
    try:
        payload = state.get_config_payload()
...
def test_dashboard_config_payload_includes_strategy11_fields(tmp_path: Path):
    ...
    try:
        payload = state.get_config_payload()
        assert payload['strategy_catalog']['11']['label'] == 'BTC 概率定价'
        assert payload['labels']['STRATEGY11_MIN_EDGE'] == '策略11 最小概率优势'
        assert payload['labels']['STRATEGY11_VOLATILITY_BPS_PER_SQRT_MINUTE'] == '策略11 波动率估计'
        assert payload['field_scope']['STRATEGY11_MIN_EDGE'] == 'strategy_11_only'
        assert payload['field_scope']['STRATEGY11_CONFIRM_BEFORE_ENTRY_SECONDS'] == 'strategy_11_only'
        assert 'STRATEGY11_MIN_EDGE' not in payload['editable_keys']
        assert 'STRATEGY11_CONFIRM_BEFORE_ENTRY_SECONDS' not in payload['editable_keys']
    finally:
        state.close()
...
def test_dashboard_update_config_accepts_strategy11_values(tmp_path: Path):
    env_file = tmp_path / '.env.dashboard'
    state = DashboardState(env_file=env_file)
    try:
        payload = state.update_config({
...
def test_dashboard_assets_include_strategy7_copy_and_reasons(tmp_path: Path):
    ...
    try:
        ...
        assert payload['strategy_catalog']['7']['label'] == '盘口+动量共识'
    assert payload['strategy_catalog']['7']['summary'] == '币安盘口失衡与预测市场动量必须同向确认，冲突时跳过。'
    assert 'strategy7_signal_conflict' in js
    assert 'strategy7_confidence_too_low' in js
    assert payload['strategy_catalog']['9']['label'] == '稳定共振'
    assert payload['strategy_catalog']['9']['summary'] == '在策略7共识基础上加入连续稳定、衰减风险与动态价帽过滤。'
    assert 'strategy9_signal_unstable' in js
    assert 'strategy9_dynamic_price_too_high' in js
